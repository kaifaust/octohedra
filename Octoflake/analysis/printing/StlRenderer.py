...
class StlRenderer:
    ...
    def render_with_config(self, grid: OctoGrid, config: OctoConfig):
        config.derive()
        return self.render(grid, config.cell_size, config.overlap, config.slit, config.floor)
    ...
    def render(self, grid, cell_size, overlap, slit, pyramid_floor=0.3):
        ...
        for center, cell in grid.occ.items():
            ...
            meshes.append(
                self.trimmed_octo(cell_size, overlap, slit, center, cell.trims, cell.is_pyramid, pyramid_floor))
